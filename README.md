# Ignited Team References

## Idea
### Cards Game(Dispatch) + City Leakage: 
#### Card Games:
(Incase we have more time + Game + Maps)
- Problems of water & How to solve them via Cards 
- Leaderboard

#### City Leakage:
1. Map (Top View)
2. Upload Linked with map
3. Problems of writer(Description)
4. User enters Location while uploading Image

![Dispatch Reference](https://tse2.mm.bing.net/th/id/OIP.FYzJPtsAiZY8o1UF0wwQzwHaEK?cb=defcachec2&rs=1&pid=ImgDetMain&o=7&rm=3)


## Session Tracker

### February 2026
- [x] Tue — 17/02/2026 — 5:00 PM
- [ ] Sun — 22/02/2026 — 10:00 PM (Today)
- [ ] Tue — 24/02/2026 — 10:00 PM

### March 2026
- [ ] Sun — 01/03/2026 — 10:00 PM
- [ ] Tue — 03/03/2026 — 10:00 PM
- [ ] Sun — 08/03/2026 — 10:00 PM
